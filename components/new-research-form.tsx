...
import { useCreateResearchMutation } from '@/hooks/use-research';
...
const NewResearchForm: React.FC<NewResearchFormProps> = ({ onSuccess }) => {
  const t = useTranslations(baseTPath);
  const router = useRouter();
  const { mutateAsync: createResearchMutation, isPending: isPendingCreateResearch } = useCreateResearchMutation();
  ...
  const handleSubmit = async (
    ...
    const researchDto: CreateResearchDto = {
      ...
      completedYear: values.completedYear ? parseInt(values.completedYear, 10) : undefined,
      title: values.title,
    ...
    try {
      const createdResearch = await createResearchMutation(researchDto);
      // Call parent's onSuccess with the created id
      onSuccess(createdResearch);
    } catch (error: any) {
      // Set a generic error message
      actions.setStatus({ error: error.message || "Failed to create research." });
    } finally {
            ...
            {({ values, isSubmitting, status, setFieldValue }) => (
              <Form>
                ...
                <div className="d-flex justify-content-between mt-4">
                  ...
                  <Button variant="success" type="submit" disabled={isSubmitting || isPendingCreateResearch}>
                    {isSubmitting || isPendingCreateResearch ? (
                      <>
