...
interface MediaContributionOptionsCardProps {
  mediaContribution: MediaContribution;
}

const MediaContributionOptionsCard: React.FC<MediaContributionOptionsCardProps> = ({mediaContribution}) => {
  ...
  const handleDeleteMediaContribution = async () => {
  ...
  const hasImage = !!mediaContribution.coverImage;

  return (
    <>
      <Card className="my-3 shadow media-contribution-options-card">
        <Row className="g-0">
          {hasImage && (
            <>
            {/* Mobile */}
          ...
          <Col xs={12} md={hasImage ? 8 : 12}>
            <Card.Body>
              ...
              <Card.Title>
                { mediaContribution.title.en && `${mediaContribution.title.en} ` }
              </Card.Title>
              <Card.Title>
                { mediaContribution.title.si && `${mediaContribution.title.si} ` }
              </Card.Title>
              <hr />
              {mediaContribution.description.en && (
                <Row className='mb-3'>
                  <Col>
                    <SanitizedHtml html={mediaContribution.description.en} />
                  </Col>
                </Row>
              )}
              {mediaContribution.description.si && (
                <Row className='mb-3'>
                  <Col>
                    <SanitizedHtml html={mediaContribution.description.si} />
                  </Col>
                </Row>
              )}
              <Row className="align-items-center">
                <Col className="mb-2">
                  <ButtonGroup>
                    <Button
                      variant="primary"
                      aria-label={t('read')}
                      onClick={() => router.push(`media-contributions/${mediaContribution.id}`)}
                    >
                    ...
                    <Button
                      variant="secondary"
                      aria-label={t('edit')}
                      onClick={() => router.push(`media-contributions/${mediaContribution.id}/edit`)}
                    >
                    ...
                    <Button
                      ...
                      aria-label={mediaContribution.status === DocumentStatus.ACTIVE ? t('deactivate') : t('activate')}
                      onClick={mediaContribution.status === DocumentStatus.ACTIVE ? handleDeactivate : handleActivate}
                      disabled={isPendingActivate || isPendingDeactivate}
                    >
                      <FontAwesomeIcon
                        icon={mediaContribution.status === DocumentStatus.ACTIVE ? faEyeSlash : faEye}
                        className="me-sm-1"
                      />
      ...
      <DeleteModal
        ...
        onConfirm={handleDeleteMediaContribution}
      />
    </>
  )
}

export default MediaContributionOptionsCard;