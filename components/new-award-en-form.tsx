...
import { useCreateAwardEnMutation } from '@/hooks/use-awards';
...
interface NewAwardEnFormProps {
  onSuccess: (award: Award) => void;
}

const NewAwardEnForm: FC<NewAwardEnFormProps> = ({ onSuccess }) => {
  const t = useTranslations(baseTPath);
  const router = useRouter();
  const { mutateAsync: createAwardEnMutation, isPending: isPendingCreateAward } = useCreateAwardEnMutation();
  ...
  const handleSubmit = async (
    ...
    try {
      const createdAward = await createAwardEnMutation(awardEnDto);
      // Call parent's onSuccess with the created id
      onSuccess(createdAward);
    } catch (error: any) {
      // Set a generic error message
      actions.setStatus({ error: error.message || "Failed to create award." });
    } finally {
            ...
            {({ values, setFieldValue, isSubmitting, status }) => (
              <Form>
                ...
                <div className="d-flex justify-content-between mt-4">
                  ...
                  <Button variant="success" type="submit" disabled={isSubmitting || isPendingCreateAward}>
                    {isSubmitting || isPendingCreateAward ? (
                      <>
